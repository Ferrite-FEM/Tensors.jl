...
for dim in 1:3
    ...
    for T in (Float32, Float64)
        ...
        # Hessians of scalars
        @test Δ(norm, A) ≈ Δ(norm, A, :all)[1] ≈ reshape(ForwardDiff.hessian(x -> sqrt(sumabs2(x)), A), (dim, dim, dim, dim))
        @test Array(Δ(norm, A, :all)[2]) ≈ reshape(ForwardDiff.gradient(x -> sqrt(sumabs2(x)), A), (dim, dim))
