...
# Unsafe versions that just truncate to the stored components
@inline unsafe_symmetric(S::SymmetricTensors) = S
@tensorop function unsafe_symmetric(S::Tensor{2, dim}) where {dim}
    C[i, j]::SymmetricTensor{2, dim} = S[i, j]
end
@tensorop function unsafe_symmetric(S::Tensor{4, dim}) where {dim}
    C[i, j, k, l]::SymmetricTensor{4, dim} = S[i, j, k, l]
end
